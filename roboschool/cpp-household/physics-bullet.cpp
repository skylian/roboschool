...
void World::bullet_init(float gravity, float timestep) {
    ...
    b3StateLoggingStart(
            command, STATE_LOGGING_PROFILE_TIMINGS, "chrome_tracing.json");
...
void World::clean_everything() {
    b3SubmitClientCommandAndWaitStatus(
            client, b3InitResetSimulationCommand(client));
...
shared_ptr<Robot> World::load_urdf(const std::string& fn,
    ...
    int statusType;
    b3SharedMemoryCommandHandle command =
            b3LoadUrdfCommandInit(client, fn.c_str());
    b3LoadUrdfCommandSetStartPosition(
            command, tr.getOrigin()[0], tr.getOrigin()[1], tr.getOrigin()[2]);
    b3LoadUrdfCommandSetStartOrientation(command,
                                         tr.getRotation()[0],
                                         tr.getRotation()[1],
                                         tr.getRotation()[2],
                                         tr.getRotation()[3]);
...
std::list<shared_ptr<Robot>> World::load_sdf_mjcf(const std::string& fn,
    ...
    if (mjcf) {
        b3SharedMemoryCommandHandle command =
                b3LoadMJCFCommandInit(client, fn.c_str());
...
static void load_shape_into_class(const shared_ptr<ThingyClass>& klass,
                                  int geom,
                                  const std::string& fn,
...
shared_ptr<Thingy> World::load_thingy(const std::string& the_filename,
                                      const btTransform& tr,
                                      float scale, float mass, uint32_t color,
...
void World::load_robot_joints(const shared_ptr<Robot>& robot,
                              const std::string& original_fn) {
    b3BodyInfo root;
...
shared_ptr<ThingyClass> World::klass_cache_find_or_create(
...
void World::load_robot_shapes(const shared_ptr<Robot>& robot, float scale) {
    ...
    for (int i=0; i<visualShapeInfo.m_numVisualShapes; i++) {
        ...
        if (!part->klass) {
            ...
            snprintf(klass_name,
                     sizeof(klass_name),
                     "%s:%03i", robot->original_urdf_name.c_str(), link_n);
            //fprintf(stderr,
            //        "allocating class=='%s' link==%i geom=%i fn=='%s'\n",
        ...
        if (!part->klass->frozen) {
            //fprintf(stderr,
            //        "adding visual to link==%i geom=%i fn=='%s',"
            //        " already have %i viz shapes\n",
            //        link_n, geom, fn.c_str(),
            //        (int)part->klass->shapedet_visual->detail_levels[DETAIL_BEST].size());
            load_shape_into_class(
                    part->klass, geom, fn,
        ...
        if (cx) {
            cx->need_load_missing_textures = true;
        }
    ...
    for (const weak_ptr<Robot>& wr: robotlist) {
        ...
        for (const shared_ptr<Joint>& j: robot->joints) {
            ...
            if (j->torque_need_repeat) {
                if (!cmd) {
                    cmd = b3JointControlCommandInit2(
                            client, robot->bullet_handle, CONTROL_MODE_TORQUE);
                }
                b3JointControlSetDesiredForceTorque(
                        cmd, j->bullet_uindex, j->torque_repeat_val);
            }
        }
...
static btTransform transform_from_doubles(
...
void World::query_body_position(const shared_ptr<Robot>& robot) {
    ...
    b3SharedMemoryCommandHandle cmd_handle =
            b3RequestActualStateCommandInit(client, robot->bullet_handle);
    b3RequestActualStateCommandComputeLinkVelocity(
            cmd_handle, ACTUAL_STATE_COMPUTE_LINKVELOCITY);
    ...
    robot->root_part->bullet_local_inertial_frame =
            transform_from_doubles(root_inertial_frame, root_inertial_frame+3);
    ...
    for (const shared_ptr<Thingy>& part: robot->robot_parts) {
        ...
        part->bullet_position =
                transform_from_doubles(linkstate.m_worldPosition,
                                       linkstate.m_worldOrientation);
        part->bullet_local_inertial_frame =
                transform_from_doubles(linkstate.m_localInertialPosition,
                                       linkstate.m_localInertialOrientation);
        part->bullet_link_position =
                transform_from_doubles(linkstate.m_worldLinkFramePosition,
                                       linkstate.m_worldLinkFrameOrientation);
...
void World::robot_move(const shared_ptr<Robot>& robot,
    ...
    b3SharedMemoryCommandHandle cmd =
            b3CreatePoseCommandInit(client, robot->bullet_handle);
    b3CreatePoseCommandSetBasePosition(cmd,
                                       tr.getOrigin()[0],
                                       tr.getOrigin()[1],
                                       tr.getOrigin()[2]);
    b3CreatePoseCommandSetBaseOrientation(cmd,
                                          tr.getRotation()[0],
                                          tr.getRotation()[1],
                                          tr.getRotation()[2],
                                          tr.getRotation()[3]);
...
std::list<shared_ptr<Household::Thingy>> World::bullet_contact_list(
        const shared_ptr<Thingy>& t) {
    b3SharedMemoryCommandHandle cmd =
            b3InitRequestContactPointInformation(client);
    ...
    for (int c=0; c<contacts.m_numContactPoints; c++) {
        ...
        //fprintf(stderr,
        //        "collision, asked for %02i:%02i, got %02i:%02i vs %02i:%02i\n",
        ...
        if (f == bullet_handle_to_robot.end()) {
            fprintf(stderr,
                    "World::bullet_contact_list() contact with object that was"
                    " not created via World interface.\n");
            continue;
        }
        shared_ptr<Robot> other_robot = f->second.lock();
        if (!other_robot) {
            fprintf(stderr,
                    "World::bullet_contact_list() contact with object that is"
                    " dead according to local bookkeeping.\n");
            continue;
        }
        shared_ptr<Thingy> other_part =
                link_b==-1 ? other_robot->root_part :
                             other_robot->robot_parts[link_b];
